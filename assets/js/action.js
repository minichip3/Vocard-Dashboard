...
function updatePrimaryColor(color) {
    const $colorSelectContainer = $(
        "#user-settings-page .color-select-container"
    );

    const $colorOption = $colorSelectContainer.find(
        `.color[data-id="${color}"]`
    );
    ...
    htmlElement.attr("class", function (i, c) {
        ...
        const remainingClasses = currentClasses.filter(
            (cls) => !cls.startsWith("primary-") && cls !== "light-theme"
        );

        return (
            (hasLightTheme ? "light-theme " : "") +
    ...
    const generateLyrics = () => {
        if (lyrics) {
            return (lyrics.default || [])
                .map(
                    (line) =>
                        `<p class="line">${line.replace(/\n/g, "<br>")}</p>`
                )
                .join("");
        }
        return Array.from(
            { length: DEFAULT_LYRIC_LINES },
            () => `<p class="line"></p>`
        ).join("");
    };
...
$(document).ready(function () {
    ...
    if (localStorage.getItem("theme") == "light") {
        ...
        $(
            "#user-settings-page .settings-row input[data-id='toggle-dark-mode']"
        ).prop("checked", false);
    }
    ...
    $("#main-page").on("scroll", function () {
        if (
            ...
            $(this)[0].scrollHeight * 0.85
        ) {
            let loader = $("#recommendation-tracks-loader");

            let totalTracks = $("#history-tracks, #recommendation-tracks").find(
                "[data-id]"
            ).length;
            ...
            if (loader.css("display") === "none") {
                var elements = $(
                    "#history-tracks, #recommendation-tracks"
                ).find("[data-id]");
    ...
    $(".queue-container .scrollbar").on("scroll", function () {
        ...
        $("#queue-scroll-to-top").toggleClass(
            "active",
            isScrollingUp && currentScrollTop > 0
        );
    ...
    $(document).on("click", function (e) {
        ...
        panels.forEach((panelId) => {
            if (
                ...
                !$target.closest(`#toggle-${panelId}`).length
            ) {
        ...
        if (!$target.closest(".select-container").length) {
            ...
            $(".select-container .material-symbols-outlined").removeClass(
                "rotate"
            );
        }
        ...
        if ($menuContainer.length) {
            ...
            if ($btn.length) {
                ...
                if (pageName == "create-playlist") {
                    if (player.selectedBot == undefined) {
                        return player.tm.showToast(
                            "error",
                            localeTexts.errors.noPlayerError
                        );
                    }
                    return $("html").append(
                        buildModalHtml({
                            ...
                            header: {
                                ...
                                description:
                                    localeTexts.playlist.create.description,
                            },
                            fields: {
                                playlistName: {
                                    title: localeTexts.playlist.variable.name,
                                    placeholder:
                                        localeTexts.playlist.variable
                                            .namePlaceholder,
                                    inputType: "text",
                                    maxLength: 20,
                                    disable: false,
                                ...
                                playlistType: {
                                    ...
                                    options: {
                                        standard: {
                                            title: localeTexts.playlist.variable
                                                .standard,
                                            triggerClass: [],
                                        },
                                        online: {
                ...
                if (playlistPage.length > 0) {
                    $(`#${pageId}`).replaceWith(
                        buildPlaylistHtml(dataId, playlist, "user-playlist")
                    );
            ...
            if ($track.length) {
                ...
                if ($target.is("span")) {
                    if (
                        ...
                        ["link", "share"].includes(
                            player.playlists[options.selectedPlaylistId]?.type
                        )
                    ) {
                    ...
                    if (currentTrack != undefined) {
                        if ($target.hasClass("filled")) {
                            player.send({
                                ...
                                type: "removeTrack",
                                playlistId: "200",
                                trackPosition: player.playlists[
                                    "200"
                                ]?.tracks.indexOf(currentTrack.trackId),
            ...
            if ($target.closest("#queue-scroll-to-top").length) {
                ...
                if ($track.length) {
                    const options = {
                        selectedTrackId: $track.data("id"),
                        selectedTrackIndex:
                            $track.index("#history-queue .track") + 1,
                    };
                    if ($target.is("span")) {
                        return buildContextMenu(
                            e,
        ...
        if ($target.closest("#settings-page").length) {
            ...
            if ($targetSettings.hasClass("access-server-settings")) {
                $(".main-container").append(
                    buildSettingPageHtml(guildId, { fields: {} })
                );
        ...
        if ($settingsContainer.length) {
            ...
            if ($selectContainer.length) {
                ...
                if ($option.length) {
                    ...
                    if (
                        player.currentSettings.settings[dataId] !==
                        selectedValue
                    ) {
        ...
        if ($togglePanel.length) {
            const targetPanelId = $togglePanel
                .attr("id")
                .replace("toggle-", "");
            switch (targetPanelId) {
                case "history-list":
                    ...
                    const isHistoryQueueInvisible =
                        !$historySection.is(":visible");

                    if (
                        isHistoryQueueInvisible &&
        ...
        if ($modal.length) {
            ...
            if ($selectContainer.length) {
                ...
                if ($option.length) {
                    ...
                    const selectedValue = $option.text();
                    const selectedTriggerClass = $option
                        .data("trigger")
                        .split(",");
                    const $selectedContainer = $selectContainer.find(
                        ".selected-container"
                    );
        ...
        if ($userSettingsContainer.length) {
            ...
            const $colorSelectContainer = $target.closest(
        ...
        switch (id) {
            ...
            case "context-playlist-play-shuffle":
                var $playlistContainer = $(
                    `#playlist-page-${options.selectedplaylistid}`
                );
                if ($playlistContainer.length) {
            ...
            case "context-get-lyrics":
                ...
                $(".main-container").append(
                    buildLyricHtml(pageId, { title: track.title })
                );
                ...
                $("html").append(
                    buildModalHtml({
                        ...
                        header: {
                            ...
                            description:
                                localeTexts.playlist.rename.description,
                        },
                        fields: {
                            playlistName: {
                                title: localeTexts.playlist.variable.name,
                ...
                $("html").append(
                    buildModalHtml({
                        ...
                        header: {
                            ...
                            description:
                                localeTexts.playlist.delete.description,
                        },
                        fields: {},
                        footer: {
                            submit: {
            ...
            case "context-playlist-create":
                ...
                op = {
                    op: "updatePlaylist",
                    type: "createPlaylist",
                    playlistName: `link-${options.selectedplaylisthref.slice(
                        -5
                    )}`,
    ...
    function buildContextMenu(event, buttonNames, options) {
        const buttonActions = {
            playNow: createActionElement(
                "play_arrow",
                localeTexts.context.playNow,
                "context-play-now"
            ),
            playNext: createActionElement(
                "redo",
                localeTexts.context.playNext,
                "context-play-next"
            ),
            addToQueue: createActionElement(
                "add_notes",
                localeTexts.context.addToQueue,
                "context-play-add-to-queue"
            ),
            removeTrack: createActionElement(
                "do_not_disturb_on",
                localeTexts.context.removeFromQueue,
                "context-remove-track",
                !player.isDJ,
                true
            ),
            skipTo: createActionElement(
                "skip_next",
                localeTexts.context.skipTo,
                "context-skip-to",
                options.selectedTrackIndex !== 1 || !player.isDJ
            ),
            backTo: createActionElement(
                "skip_previous",
                localeTexts.context.backTo,
                "context-back-to",
                options.selectedTrackIndex !== 1 || !player.isDJ
            ),
            ...
            copyLink: createActionElement(
                "content_copy",
                localeTexts.context.copyLink,
                "context-copy-link"
            ),
            getRecommendation: createActionElement(
                "thumb_up",
                localeTexts.context.recommendation,
                "context-get-recommendation"
            ),
            getLyrics: createActionElement(
                "lyrics",
                localeTexts.context.getLyrics,
                "context-get-lyrics"
            ),
            playlistShuffle: createActionElement(
                "shuffle",
                localeTexts.context.playShuffle,
                "context-playlist-play-shuffle"
            ),
            playlistRemoveTrack: createActionElement(
                "do_not_disturb_on",
                localeTexts.context.removeTrack,
                "context-playlist-remove-track",
                false,
                true
            ),
            playlistRemove: createActionElement(
                "playlist_remove",
                localeTexts.playlist.delete.title,
                "context-playlist-remove"
            ),
            playlistRename: createActionElement(
                "bookmark",
                localeTexts.playlist.rename.title,
                "context-playlist-rename"
            ),
            playlistCreate: createActionElement(
                "playlist_add",
                localeTexts.playlist.create.title,
    ...
    function showContextMenu(e, contextMenuHtml, options) {
        ...
        const contextMenu = $(`<div class="context-menu"></div>`).html(
            contextMenuHtml
        );
    ...
    function buildTrackPage(trackId) {
        ...
        $trackPageHtml =
            $(`<div class="sections" id="${trackPageId}" data-id="${trackId}">
            <div class="section">
                <div class="track-header">
                    <div class="left">
                        <img src="${
        ...
        $trackPageHtml.on("scroll", function () {
            ...
            let totalTracks = $trackPage.find(
                ".recommendation-tracks [data-id]"
            ).length;

            if (totalTracks > maxRecommendationTracks) return;

            if (
                $trackPage.scrollTop() + $trackPage.innerHeight() >=
                $trackPage[0].scrollHeight * 0.85
            ) {
                if (loader.css("display") === "none") {
                    let recommendationTrack = $trackPage.find(
